...
// ==========================================
// 🔥 核心：理解用戶意圖（交給 DeepSeek）
// ==========================================
async function understandUserIntent(userMessage, session) {
    const hasQuestions = session.diaryQuestionQueue && session.diaryQuestionQueue.length > 0;
    const mode = session.diaryMode || 'rest';
    const isFaith = session.isFaithMode || false;

    const systemPrompt = `
你是咩咩的意圖理解模組。請根據用戶說的話，判斷他現在想做什麼。

${DIARY_RULES}

【當前狀態】
- 是否有未回答的題目：${hasQuestions ? '有' : '無'}
- 當前日記模式：${mode === 'busy' ? '忙碌模式（2題）' : '安息模式（4題）'}
- 當前信仰模式：${isFaith ? '信仰模式' : '非信仰模式'}

【請判斷用戶意圖，只返回 JSON，不要有其他內容】
可選意圖：
- "出題"：用戶想要出題目（例如：給我試題、出題、我要題目、今天寫點東西、來點日記題）
- "回答題目"：用戶正在回答當前的日記題目
- "切換模式"：用戶想要切換忙碌/安息模式
- "切換信仰"：用戶想要切換信仰/非信仰模式
- "結束日記"：用戶今天不想寫了（例如：累了不想寫了、今天不寫了、算了）
- "重設"：用戶想要重新開始日記
- "閒聊"：用戶說了跟日記無關的話，只是在隨便聊聊
...
app.post('/api/chat', async (req, res) => {
    ...
    // ==========================================
    // 🔥 日記模組：交給 DeepSeek 理解意圖
    // ==========================================
    if (session.module === 'diary') {
        // 先檢查當前是否有未回答的題目
        const hasQuestions = session.diaryQuestionQueue && session.diaryQuestionQueue.length > 0;

        // 如果有題目，且用戶說的話很短（可能是回答），優先當作回答處理
        // 但如果用戶明確說「出題」「給我試題」等，還是要識別為出題
        // 交給 DeepSeek 判斷
        const intent = await understandUserIntent(message, session);

        // 根據意圖執行
        if (intent.intent === '出題') {
            // 提取用戶指定的數量
            const numberMatch = message.match(/(\\d+)\\s*[題题]/);
            let count = null;
            if (numberMatch) {
                const c = parseInt(numberMatch[1]);
                if (c >= 1 && c <= 10) {
                    count = c;
                }
            }
            ...
            const diaryData = generateDiaryQuestions(userId, userState, session.diaryMode || 'rest', session.isFaithMode || false, count);
            session.diaryQuestionQueue = diaryData.questions;
            const currentQuestion = session.diaryQuestionQueue.shift();
            const countText = count || diaryData.count;

            return res.json({
                reply: `已切换至「${countText}题模式」。\\n\\n${currentQuestion || '今天有沒有發生一件具體的事，讓你心裡輕輕動了一下的？'}`,
                currentLayer: 0
            });
        }

        if (intent.intent === '切換模式') {
            // 從用戶的話判斷要切換到哪個模式
            if (message.includes('忙碌') || message.includes('busy') || message.includes('少')) {
                session.diaryMode = 'busy';
            } else {
                session.diaryMode = 'rest';
            }
            const modeText = session.diaryMode === 'busy' ? '忙碌模式（2题）' : '安息模式（4题）';
            return res.json({
                reply: `已切换至「${modeText}」。`,
                currentLayer: 0
            });
        ...
        if (intent.intent === '切換信仰') {
            session.isFaithMode = !session.isFaithMode;
            const faithText = session.isFaithMode ? '信仰模式' : '非信仰模式';
            return res.json({
                reply: `已切换至「${faithText}」。`,
                currentLayer: 0
            });
        }

        if (intent.intent === '重設') {
            session.diaryQuestionQueue = [];
            session.diaryAnswers = [];
            session.yesterdayDiary = null;
            return res.json({
                reply: '重新开始。\\n\\n今天有沒有發生一件具體的事，讓你心裡輕輕動了一下的？',
                currentLayer: 0
            });
        }

        if (intent.intent === '結束日記') {
            const answers = session.diaryAnswers || [];
            if (answers.length > 0) {
                const insight = await generateDiaryInsight(answers, userId);
                session.diaryAnswers = [];
                session.diaryQuestionQueue = [];
                saveConversation(userId, 'assistant', insight);
                return res.json({ reply: insight });
            }
            return res.json({
                reply: '好的，今天先到這裡。我在。明天見。',
                currentLayer: 0
            });
        }
        ...
        if (intent.intent === '閒聊') {
            // 用閒聊方式回應
            const chatReply = await callDeepSeek([
                { role: 'system', content: CHAT_SYSTEM_PROMPT },
                { role: 'user', content: message }
            ], 0.7);
            saveConversation(userId, 'assistant', chatReply);
        ...
        // 檢查是否所有題目都答完了
        if (session.diaryQuestionQueue.length === 0) {
            const answers = session.diaryAnswers || [];
            if (answers.length > 0) {
                const insight = await generateDiaryInsight(answers, userId);
                session.diaryAnswers = [];
        ...
        // 顯示下一題
        const nextQuestion = session.diaryQuestionQueue[0];
        return res.json({
            reply: nextQuestion,
            currentLayer: 0
        });
